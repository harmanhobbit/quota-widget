...
export class LogicalPosition { constructor(x,y){ this.x=x; this.y=y; } }`);
  ...
  // export/import controls (issue #151). Stubbed the same way as the updater
  // above rather than left to resolve the real package: a real dialog would
  // call `window.__TAURI_INTERNALS__.invoke`, which doesn't exist under jsdom.
...
export async function save(options) {
  (globalThis.__SMOKE_DIALOG_CALLS__ ??= []).push({ fn: 'save', options });
  return globalThis.__SMOKE_SAVE_DIALOG_PATH__ ?? null;
}
export async function open(options) {
  (globalThis.__SMOKE_DIALOG_CALLS__ ??= []).push({ fn: 'open', options });
  return globalThis.__SMOKE_OPEN_DIALOG_PATH__ ?? null;
}`);
}
