...
| **Moonshot / Kimi** | An API key from [platform.kimi.ai](https://platform.kimi.ai), or a Kimi Code subscription | **Open Platform API key** (the default) calls official `GET /v1/users/me/balance` and shows `available_balance` in USD. Keys are platform-specific: a `platform.kimi.com` key needs **Balance URL** pointed at that host. **Kimi Code subscription** (desktop) uses a built-in device sign-in and reports its weekly and per-window quotas from `GET https://api.kimi.com/coding/v1/usages`. This OAuth flow and endpoint are reimplemented from the [published Kimi Code CLI](https://github.com/MoonshotAI/kimi-code), rather than documented Open Platform APIs, so they may change. |
